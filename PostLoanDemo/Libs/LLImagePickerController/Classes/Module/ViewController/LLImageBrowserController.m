...
@property (nonatomic, strong) PHImageModel *phImageModel;
...
{
    AVPlayerItem *playerItem = [AVPlayerItem playerItemWithURL:self.phImageModel.url];
    //2、创建播放器
    ...
    //临时添加，以后换成图片
    lab = [[UILabel alloc] initWithFrame:CGRectMake(20, 120, 50, 30)];
    lab.text = @"播放";
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.pickerType == 1) {
       ...
       #pragma mark - 播放视频 add by pzj 2017-10
        WeakSelf(self)
        [cell reloadDataWithMoviePHAsset:self.phAssetsArray[indexPath.row] resultHandler:^(PHImageModel *model, PHAsset *phAsset) {
            NSLog(@"视频model = %@",model);
            weakSelf.phImageModel = model;
            [weakSelf playVideoMethod];
        }];
        [cell handleSingleTapActionWithBlock:^{
            if (isSelect) {
                lab.text = @"播放";
...
- (void)setIsOnlyBrowse:(BOOL)isOnlyBrowse
{
    _isOnlyBrowse = isOnlyBrowse;
}
#pragma mark -
